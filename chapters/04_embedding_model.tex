...
Three baselines are used. The \emph{Frequency-Based Model} (FBM) is the
incumbent frequency/co-occurrence recommender: an $n\times n$ matrix over
products in which entry $m_{ij}$ is the number of observed transitions from
product $i$ to product $j$, with each row normalised to sum to one.
\emph{Backfilling} is a coverage mechanism layered on top --- for a product with
