%==============================================================
%  Chapter 3 -- Position Bias: Estimation and Correction
%  Source: Semester-2 mid-term deck (position-bias pages only;
%  two-stage model, its CTR tables, and the live-eval table are
%  intentionally excluded). The K-step recommender is introduced
%  only briefly, as the existing engine into which the estimated
%  bias is integrated.
%==============================================================
...
\begin{figure}[ht]
  \centering
   \includegraphics[width=0.85\textwidth]{figures/pos.png}
  % \fbox{\parbox[c][3.5cm][c]{0.85\textwidth}{\centering\itshape
  %   [Figure to be inserted] Distribution of clicks by clicked
  %   recommendation position (modid=IM): clicks decay monotonically from
  %   rank~1 ($\approx 7.4$k) to rank~6 ($\approx 4.3$k).}}
  \caption{Observed clicks by position. The monotone decline with rank is
           direct evidence of position bias.}
  \label{fig:pb-clicks.png}
\end{figure}
...
\begin{figure}[ht]
  \centering
  % \includegraphics[width=0.85\textwidth]{figures/pos1_vs_others.png}
  \fbox{\parbox[c][3.5cm][c]{0.85\textwidth}{\centering\itshape
    [Figure to be inserted] Weekly normalised positional performance vs.\
    position~1 (top 25\% by volume): effectiveness ratio $\pi_1/\pi_i$ for
    $i\in\{2,3,4,5\}$ across Jul~2024--May~2025.}}
...
\begin{figure}[ht]
  \centering
  % \includegraphics[width=0.85\textwidth]{figures/adjusted_cg_ratio.png}
  \fbox{\parbox[c][3.5cm][c]{0.85\textwidth}{\centering\itshape
    [Figure to be inserted] Weekly adjusted quality ratio per position with
    Google search as the reference anchor (CG), top 25\% by volume.}}
...
\begin{figure}[ht]
  \centering
  % \includegraphics[width=0.85\textwidth]{figures/adjusted_cim_ratio.png}
  \fbox{\parbox[c][3.5cm][c]{0.85\textwidth}{\centering\itshape
    [Figure to be inserted] Weekly adjusted quality ratio per position with
    IndiaMART search as the reference anchor (CIM), top 25\% by volume.}}
...
refreshing them periodically; those refresh experiments are reported
separately.

\begin{figure}[ht]
  \centering
  % \includegraphics[width=0.9\textwidth]{figures/mismatches.png}
  \fbox{\parbox[c][3.5cm][c]{0.9\textwidth}{\centering\itshape
    [Figure to be inserted] Daily mismatch \% (rising $0\to40\%$) against
    aggregate newly-found \% (falling $85\to25\%$) over Jul~2024--May~2025,
    top 25\% by volume.}}
